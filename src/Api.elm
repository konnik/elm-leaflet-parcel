...
nyFors session fors toMsg =
    post session
        ...
        , body = Http.jsonBody (forsTillJson Nothing fors)
        }
...
uppdateraFors session (Resurs id fors) toMsg =
    ...
    put session
        { url = "https://forsguiden-api.herokuapp.com/forsstracka/" ++ String.fromInt id
        , expect = Http.expectJson toMsg (resurs forsDecoder)
        , body = Http.jsonBody (forsTillJson (Just id) fors)
        }


forsTillJson : Maybe Int -> Fors -> E.Value
forsTillJson maybeId fors =
    E.object <|
        List.concat
            [ maybeId |> Maybe.map (\id -> [ ( "id", E.int id ) ]) |> Maybe.withDefault []
            , [ ( "namn", E.string fors.namn )
              , ( "gradering"
                , E.object
                    [ ( "klass", E.string <| gradToString fors.gradering.klass )
